...
const bubbleCategories = [
  ...
  { id: "video", label: "视频制作", type: "works", desc: "二创混剪、原创剧情片、分镜与剪辑作品。" },
...
const worksData = {
  ...
  video: {
    intro: "这里展示 Hetty 的视频制作作品。",
    subcategories: {
      fanEdit: {
        title: "二创混剪",
        intro: "这里收集追星向剪辑、情绪向混剪、节奏卡点、人物高光片段和氛围感视频。",
        items: [
          {
            id: "fanedit-001",
            bubbleText: "混剪 01",
            ...
            date: "2026",
            role: "剪辑 / 节奏 / 调色",
            intro: "以人物高光和情绪节奏为核心的混剪作品。",
            highlight: "先显示封面，点击后用 HTML5 video 播放本地 MP4。",
            videoUrl: "assets/videos/fanedit-01.mp4",
            link: "#"
          },
          {
            id: "fanedit-002",
            ...
            role: "剪辑 / 节奏 / 调色",
            intro: "二创混剪视频作品。",
            highlight: "以视频首帧作为预览入口。",
            videoUrl: "assets/videos/fanedit-02.mp4",
            link: "#"
          },
          {
            id: "fanedit-003",
            ...
            intro: "二创混剪视频作品。",
            highlight: "以视频首帧作为预览入口。",
            videoUrl: "assets/videos/fanedit-03.mp4",
            link: "#"
          }
        ]
      },
      originalFilm: {
        title: "原创剧情片",
        intro: "这里展示原创剧情短片、分镜创作、剧本拍摄、剪辑成片等作品。",
        items: [
          {
            id: "film-001",
            bubbleText: "《Ditto-不能说的幻想秘密》",
            title: "《Ditto-不能说的幻想秘密》",
            type: "video",
            date: "2026",
            role: "编剧/导演/拍摄/剪辑",
            intro: "青春里，林颂用摄影机记录下她的幻想秘密。",
            highlight: "本地视频路径固定从 assets/videos/ 读取。",
            cover: "assets/images/film-01-cover.jpg",
            images: ["assets/images/film-01-cover.jpg"],
            videoUrl: "assets/videos/film-01.mp4",
            videoCover: "assets/images/film-01-cover.jpg",
            link: "#"
          },
          {
            id: "film-002",
            ...
            date: "2026",
            role: "编剧/导演/拍摄/监制",
            intro: "自习室里，小纸条和贴心物件传递者陌生但温暖的善意。",
            highlight: "本地视频路径固定从 assets/videos/ 读取。",
            cover: "assets/images/film-02-cover.jpg",
            images: ["assets/images/film-02-cover.jpg"],
            videoUrl: "assets/videos/film-02.mp4",
            videoCover: "assets/images/film-02-cover.jpg",
            link: "#"
          }
        ]
      }
...
function renderSubcategory(categoryId, subcategoryId) {
  ...
  if (categoryId === "video" && subcategoryId === "fanEdit") {
    renderFanEditPanel(sub);
    return;
  }
...
function renderFanEditPanel(sub) {
  const first = sub.items[0];
  $("category-content").innerHTML = `
    <p class="action-hint">点击作品查看完整内容</p>
    <div class="fanedit-panel">
      <div class="fanedit-list" aria-label="二创混剪视频入口">
        ${sub.items.map((work, index) => `
          <button class="fanedit-entry ${index === 0 ? "active" : ""}" type="button" data-video-src="${work.videoUrl}" data-video-title="${work.title}" data-video-poster="${work.videoCover || ""}">
            <span>${work.title}</span>
            <small>视频播放入口</small>
          </button>`).join("")}
      </div>
      <div class="fanedit-player">
        ${first ? renderInlineVideo(first.videoUrl, first.title, first.videoCover) + `<p id="fanedit-title">${first.title}</p>` : `<div class="empty-state">视频暂未补充</div>`}
      </div>
    </div>`;
  document.querySelectorAll(".fanedit-entry").forEach(btn => {
    btn.addEventListener("click", () => {
      document.querySelectorAll(".fanedit-entry").forEach(item => item.classList.remove("active"));
      btn.classList.add("active");
      const video = $("fanedit-video");
      const title = $("fanedit-title");
      if (video) {
        video.innerHTML = `<source src="${btn.dataset.videoSrc}">视频暂未补充`;
        if (btn.dataset.videoPoster) video.setAttribute("poster", btn.dataset.videoPoster);
        else video.removeAttribute("poster");
        video.load();
      }
      if (title) title.textContent = btn.dataset.videoTitle || "视频播放入口";
    });
  });
  lockVideoDownloads();
}
...
function renderWorkMedia(work) {
  ...
  const videoHtml = work.videoUrl ? `<button type="button" class="video-cover" data-video="${work.videoUrl}" style="background-image:linear-gradient(rgba(20,30,70,.25),rgba(20,30,70,.25)),url('${work.videoCover || work.cover || ""}')">▶ 点击播放视频</button>` : "";
  ...
  return imgHtml + videoHtml + audioHtml;
}
...
function renderInlineVideo(src, title, poster = "") {
  const posterAttr = poster ? ` poster="${poster}"` : "";
  return `<video id="fanedit-video" controls controlsList="nodownload" disablePictureInPicture preload="metadata"${posterAttr} aria-label="${title}"><source src="${src}">视频暂未补充</video>`;
}
...
function openModal(html) {
  ...
  root.querySelectorAll("[data-video]").forEach(btn => btn.addEventListener("click", () => openVideo(btn.dataset.video)));
...
function openVideo(url) {
  ...
  if (isVideoFile) {
    openModal(`<div class="modal-card"><video controls controlsList="nodownload" disablePictureInPicture autoplay preload="metadata" style="width:100%;border-radius:12px"><source src="${url}">视频暂未补充</video><div class="modal-actions"><button class="pixel-button primary" type="button" data-close-modal>关闭</button></div></div>`);
  } else {
    openModal(`<div class="modal-card"><h3>视频链接</h3><p>该作品配置了视频链接，可点击下方按钮打开。</p><div class="modal-actions"><a class="pixel-button primary" href="${url}" target="_blank" rel="noopener">打开视频</a><button class="pixel-button" type="button" data-close-modal>关闭</button></div></div>`);
  }
}
